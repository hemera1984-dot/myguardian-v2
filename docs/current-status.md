...
| 브리핑 (brief/) | 1차 완성 (Codex 검수 반영) | 상담·교육·강의 3모드 발표 도구. 발표자·청중 두 창 동기화(페이지·포인터·영상, BroadcastChannel + 수신 검증), 로컬 문서 열기. 사이드바 9번째 메뉴. 2차(원격 동기화)는 미착공 |
| 우리팀 (team/) | 팀 관리 도구 개편 | 주간 확인표(팀원별 스케줄) + 월간 팀 캘린더(케어 발행 자동 표시) + 과제 목록 + 공지. members·schedule 실데이터 입력은 사용자 몫 |
...
- scripts/verify_brief.py — 브리핑 동기화 자동 검증 12항목(페이지·포인터·영상 실이동·늦은 입장·선개설·기형 메시지 내성·로컬 동선·스크립트 비노출·콘솔) + 3해상도 스크린샷
