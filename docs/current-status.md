...
> **먼저 읽으십시오 (2026-08-12)** — 저장소 이력이 다시 쓰였습니다(강제 푸시).
> 이 저장소를 열어 둔 세션은 `git fetch origin && git reset --hard origin/main`을 먼저 실행하십시오.
> 그냥 `git pull` 하면 충돌하거나 옛 이력을 되돌려 올립니다. 자세한 사정은 `docs/handoff.md` 맨 위.
